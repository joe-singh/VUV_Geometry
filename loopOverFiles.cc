...
double photonTracker(std::string fname, double normalAngle, TFile* wavelengthFile) {
  char wavelengthPlotName[1000];
  sprintf(wavelengthPlotName, "no_cylinder_angle_%d", normalAngle);
...
void loopOverFiles() {
  ...
  for (int sampleAngle = 0; sampleAngle < 5; sampleAngle += 5) {
    ...
    for (int normal = 0; normal < 90; normal += 5) {
      ...
      char filename[1000];
      sprintf(filename, "./zero_angle_sweep/no_cylinder_angle_0.%d.root", normal);
      double ratio = photonTracker(filename, normal, wavelengthFile); 
